...
let titles = [];
let secondaryTitles = [];
let titlesContents = ["B U L J O N", "TOP DONATOR\n", "LAST FOLLOW\n"];
let recentFollow;
let recent_subs = [];
let topDonator = {
  name: "poges1",
...
var newFollow = false;
var newTopTip = false;
let secondaryContents = ["Content1", "cONTENT2"];
try {
  //config
  var loader = new THREE.FontLoader();
  var loader_texture = new THREE.TextureLoader();
  var particleTexture = loader_texture.load(
    "https://raw.githubusercontent.com/Calinou/kenney-particle-pack/master/addons/kenney_particle_pack/circle_04.png"
  );
  var typeface =
  ...
  var light2 = new THREE.PointLight(0xffffff, 5000);
  scene.add(light);
  scene.add(light2);
  ...
  loader.load(typeface, (font) => {
    titlesContents.forEach((x, idx) => {
      ...
      //pMaterial.uniforms.sprite.value = particleTexture;
  ...
  function resampleSecondaryTitle() {
    ...
    if (!shape) {
      var newMesh = new THREE.Mesh(new THREE.SphereBufferGeometry(7.5, 50, 50));
